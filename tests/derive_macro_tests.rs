...
mod derive_macro_tests {
    ...
    struct AttributeCombinations {
        ...
        #[llm(description = "Field with description and example", 
        ...
        #[llm(description = "Optional field with example", 
    ...
    struct BackwardsCompatibility {
        ...
        #[llm(description = "Old array syntax with single quotes", 
              example = "['item1', 'item2', 'item3']")]
        old_single_quotes: Vec<String>,
        
        // Using the old string-based array syntax with double quotes
        #[llm(description = "Old array syntax with double quotes", 
              example = "[\"item1\", \"item2\", \"item3\"]")]
    ...
    fn test_string_formatting() {
        ...
        assert_eq!(quoted, "This is a \"quoted\" string with 'apostrophes' and backslash \\ characters");
    ...
    fn test_attribute_combinations() {
        ...
        // Field with only description
        assert!(props["desc_only"].as_object().unwrap().contains_key("description"));
        assert!(!props["desc_only"].as_object().unwrap().contains_key("example"));
        assert!(!props["desc_only"].as_object().unwrap().contains_key("examples"));
        
        // Field with description and example
        assert!(props["desc_and_example"].as_object().unwrap().contains_key("description"));
        assert!(props["desc_and_example"].as_object().unwrap().contains_key("example"));
        assert!(!props["desc_and_example"].as_object().unwrap().contains_key("examples"));
        
        // Field with description and examples
        assert!(props["desc_and_examples"].as_object().unwrap().contains_key("description"));
        assert!(!props["desc_and_examples"].as_object().unwrap().contains_key("example"));
        assert!(props["desc_and_examples"].as_object().unwrap().contains_key("examples"));
        
        // Optional field with example
        assert!(props["optional_with_example"].as_object().unwrap().contains_key("description"));
        assert!(props["optional_with_example"].as_object().unwrap().contains_key("example"));
