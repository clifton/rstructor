...
mod llm_model_tests {
    ...
    impl TestModel {
        fn validate(&self) -> rstructor::Result<()> {
            if self.name.is_empty() {
                return Err(RStructorError::ValidationError("Name cannot be empty".to_string()));
            }
            
            if self.age < 18 {
                return Err(RStructorError::ValidationError("Age must be at least 18".to_string()));
            }
    ...
    fn test_llm_model_default_validate() {
        ...
        let model = SimpleModel { value: "test".to_string() };
