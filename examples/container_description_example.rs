...
struct Book {
    ...
    #[llm(description = "Year the book was published", 
          example = 1954)]
...
fn main() {
    ...
    println!("Book Schema:");
    println!("{}", serde_json::to_string_pretty(book_schema.to_json()).unwrap());
    
    // Generate the schema for the BookStatus enum
    let status_schema = BookStatus::schema();
    println!("\nBookStatus Schema:");
    println!("{}", serde_json::to_string_pretty(status_schema.to_json()).unwrap());
}