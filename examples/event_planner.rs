use rstructor::{LLMModel, OpenAIClient, OpenAIModel, AnthropicClient, AnthropicModel, RStructorError};
...
struct Contact {
    #[llm(description = "Name of the contact person", 
          example = "John Smith")]
    name: String,
    ...
    #[llm(description = "Role or relationship", 
...
struct Location {
    #[llm(description = "Name of the venue or location", 
    ...
    #[llm(description = "City", 
          example = "New York")]
    city: String,
    
    #[llm(description = "State or province", 
          example = "NY")]
    state: Option<String>,
    
    #[llm(description = "Postal/ZIP code", 
          example = "10001")]
    zip: Option<String>,
    
    #[llm(description = "Country", 
          example = "USA")]
    country: Option<String>,
    
    #[llm(description = "Any special instructions for finding or accessing the location",
...
struct Activity {
    #[llm(description = "Name or title of the activity", 
          example = "Welcome Reception")]
    name: String,
    
    #[llm(description = "Start time in HH:MM format", 
          example = "18:30")]
    start_time: String,
    
    #[llm(description = "End time in HH:MM format", 
          example = "20:00")]
    end_time: String,
    
    #[llm(description = "Description of the activity", 
          example = "Casual networking with drinks and appetizers")]
    description: Option<String>,
    
    #[llm(description = "Location of this activity, if different from main event", 
...
struct EventPlan {
    ...
    #[llm(description = "Type of event", 
          example = "Party")]
    event_type: String,
    
    #[llm(description = "Description of the event", 
          example = "Annual celebration for employees and their families")]
    description: String,
    
    #[llm(description = "Date of the event in YYYY-MM-DD format", 
          example = "2023-12-15")]
    date: String,
    ...
    #[llm(description = "Estimated number of attendees", 
          example = 100)]
    ...
    #[llm(description = "Any special requirements or notes", 
          example = "Need vegetarian food options")]
    special_requirements: Option<String>,
    
    #[llm(description = "Estimated budget in dollars", 
          example = 5000)]
    ...
    fn validate(&self) -> rstructor::Result<()> {
        ...
        if NaiveDate::parse_from_str(&self.date, "%Y-%m-%d").is_err() {
            return Err(RStructorError::ValidationError(
                format!("Invalid date format: {}. Expected YYYY-MM-DD", self.date)
            ));
        }
        ...
        let validate_time = |time: &str| -> rstructor::Result<()> {
            if NaiveTime::parse_from_str(time, "%H:%M").is_err() {
                return Err(RStructorError::ValidationError(
                    format!("Invalid time format: {}. Expected HH:MM", time)
                ));
            }
            Ok(())
        };
        ...
        for activity in &self.activities {
            ...
            if activity_start < event_start || activity_end > event_end {
                return Err(RStructorError::ValidationError(
                    format!("Activity '{}' time ({}-{}) is outside event hours ({}-{})",
...
async fn process_event_request(client: &impl rstructor::LLMClient, description: &str) -> Result<EventPlan> {
    let prompt = format!(
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Please describe the event you want to plan (type 'done' on a new line when finished):");
