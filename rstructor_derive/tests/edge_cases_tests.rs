...
fn test_empty_attributes() {
    ...
    assert!(schema_json["properties"].as_object().unwrap().contains_key("field_no_attrs"));
    
    // Field with empty description should have an empty description
    assert_eq!(schema_json["properties"]["field_empty_desc"]["description"], "");
...
fn test_single_field() {
    ...
    assert!(schema_json["properties"].as_object().unwrap().contains_key("only_field"));
...
fn test_unusual_field_names() {
    ...
    // Check fields exist
    assert!(schema_json["properties"].as_object().unwrap().contains_key("field_with_underscores"));
    assert!(schema_json["properties"].as_object().unwrap().contains_key("field123"));
    
    // Our implementation currently doesn't respect serde rename attributes
    assert!(schema_json["properties"].as_object().unwrap().contains_key("internal_name"));
}
...
fn test_numeric_edge_cases() {
    ...
    assert_eq!(schema_json["properties"]["large_int"]["example"], 2147483647);
    assert_eq!(schema_json["properties"]["small_int"]["example"], -2147483648);
    ...
    assert!(schema_json["properties"]["very_small_float"]["example"].as_f64().unwrap() > 0.0);
    assert!(schema_json["properties"]["very_small_float"]["example"].as_f64().unwrap() < 0.000001);
    
    // Very large float should be representable
    assert!(schema_json["properties"]["very_large_float"]["example"].as_f64().unwrap() > 1000000000.0);
}